...
export async function getTasks(): Promise<Task[]> {
  await ensureSchema();
  const rows =
    (await sql`SELECT id, title, description, "column", priority, "order", created_at FROM tasks ORDER BY "order" ASC`) as DbRow[];
...
export async function getTaskById(id: number): Promise<Task | null> {
  await ensureSchema();
  const rows =
    (await sql`SELECT id, title, description, "column", priority, "order", created_at FROM tasks WHERE id = ${id}`) as DbRow[];
